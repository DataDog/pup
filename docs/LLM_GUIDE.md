...
unknown. `--timeseries-interval` accepts `168h` or `7d`; `--from`/`--to` select an
explicit measurement window without reconstructing historical entity state.
Use returned `next_request.args` to continue a query with all controls preserved.
